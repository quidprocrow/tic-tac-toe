...
var winEvent = function winEvent(boardArray) {
  var players = ['x', 'o'];
  var win = false;
  ...
  for (var i = 0; i < players.length; i++) {
    ...
    if (boardArray[0] === players[i] && boardArray[1] === players[i] && boardArray[2] === players[i]) {
      win = true;
    }
    // Horizontal win: second row.
    if (boardArray[3] === players[i] && boardArray[4] === players[i] && boardArray[5] === players[i]) {
      win = true;
    }
    // Horizontal win: third row.
    if (boardArray[6] === players[i] && boardArray[7] === players[i] && boardArray[8] === players[i]) {
      win = true;
    }
    // Vertical win: first column.
    if (boardArray[0] === players[i] && boardArray[3] === players[i] && boardArray[6] === players[i]) {
      win = true;
    }
    // Vertical win: second column.
    if (boardArray[1] === players[i] && boardArray[4] === players[i] && boardArray[7] === players[i]) {
      win = true;
    }
    // Vertical win: third column.
    if (boardArray[2] === players[i] && boardArray[5] === players[i] && boardArray[8] === players[i]) {
      win = true;
    }
    // Diagonal win: left to right.
    if (boardArray[0] === players[i] && boardArray[4] === players[i] && boardArray[8] === players[i]) {
      win = true;
    }
    // Diagonal win: right to left;
    if (boardArray[2] === players[i] && boardArray[4] === players[i] && boardArray[6] === players[i]) {
      win = true;
    }
...
var xWinEvent = function xWinEvent(boardArray) {
  var players = ['x'];
  var win = false;
  // Loops checks first to see if X has a winning combination of moves;
  // then o.
...
var setMove = function setMove(move, index) {
  ...
  var cellUpdate = JSON.stringify(arrayUpdate);
  api.updateGame(cellUpdate).then(ui.updateGameSuccess).catch(ui.updateGameFailure);
};

// Places the move on the HTML board and the array.
var boardMove = function boardMove(boxId) {
  var xHtml = '<p>X</p>';
  var oHtml = '<p>O</p>';
  var boardCell = getData(boxId);
  ...
  if (store.game.cells[boardCell] === '') {
    ...
    var invalidMoveHtml = '<B>Someone beat you to it!</B>';
...
var clickEvent = function clickEvent() {
  ...
  if (store.game.over !== true) {
    ...
    var invalidMoveHtml = '<B>Game is still over, brah.</B>';
...
var config = __webpack_require__(2);
var boardEvents = __webpack_require__(7);
var introEvents = __webpack_require__(15);
var signInEvents = __webpack_require__(16);
var signUpEvents = __webpack_require__(18);
var profileEvents = __webpack_require__(21);
var aiEvents = __webpack_require__(24);
...
var msgClear = function msgClear() {
  $('#user-msg').html('');
  $('#ai-user-msg').html('');
};

// Makes the API reflect that the game is over, shows reset button.
var markGameOver = function markGameOver(move, index) {
  var arrayUpdate = {
    game: {
      cell: {
        index: index,
  ...
  var cellUpdate = JSON.stringify(arrayUpdate);
  api.updateGame(cellUpdate).then(endGameSuccess).catch(endGameFailure);
};
...
var updateGameFailure = function updateGameFailure(data) {
  var invalidMoveHtml = '<B>How bizzare.</B>';
  $('#user-msg').html(invalidMoveHtml);
};
...
var endGameSuccess = function endGameSuccess(data) {
  ...
  var endMoveHtml = '<p><B>Game over.</B></p>';
  $('#user-msg').append(endMoveHtml);
  $('#reset-game').show();
};

var endGameFailure = function endGameFailure(data) {
...
var introSignIn = function introSignIn(event) {
...
var introSignUp = function introSignUp(event) {
  event.preventDefault();
  $('#intro').hide();
  $('#sign-in').hide();
  $('#sign-up').show();
...
var addIntroHandlers = function addIntroHandlers() {
...
module.exports = {
  addIntroHandlers: addIntroHandlers
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var signIn = function signIn(event) {
  ...
  api.signInUser(data).then(ui.signInSuccess).catch(ui.signInFailure);
...
var addSignInHandlers = function addSignInHandlers() {
  $('#sign-in-form').on('submit', signIn);
};

module.exports = {
  addSignInHandlers: addSignInHandlers
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var signInSuccess = function signInSuccess(data) {
  ...
  var userGreet = store.user.email.split('@');
  // Greet the user.
...
var signUp = function signUp(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.signUpUser(data).then(ui.signUpSuccess).catch(ui.signUpFailure);
...
var addSignUpHandlers = function addSignUpHandlers() {
  $('#sign-up-form').on('submit', signUp);
};

module.exports = {
  addSignUpHandlers: addSignUpHandlers
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var signUpSuccess = function signUpSuccess(data) {
  store.user = data.user;
  var successHtml = '<p>Success, <b>' + store.user.email + '</b>!</p>\n    <p>Please <a id="sign-in-redirect">sign in</a>.</p>\n    ';
...
var signOut = function signOut(event) {
  event.preventDefault();
  var data = store;
  api.signOutUser(data).then(ui.signOutSuccess).catch(ui.signOutFailure);
};

// User is directed to change password section,
var changePasswordRedirect = function changePasswordRedirect() {
  $('#two-player').hide();
  $('#profile').hide();
  $('#game-board').hide();
  $('#change-pass').show();
...
var playGameRedirect = function playGameRedirect() {
  $('#two-player').hide();
  $('#profile').show();
  $('#game-board').hide();
  $('#change-pass').hide();
...
var instructionsRedirect = function instructionsRedirect() {
  $('#two-player').hide();
  $('#instructions').show();
  $('#game-board').hide();
  $('#change-pass').hide();
...
var creditRedirect = function creditRedirect() {
  $('#two-player').hide();
  $('#credit').show();
  $('#instructions').hide();
  $('#game-board').hide();
...
var onStatsLoad = function onStatsLoad() {
  api.getTotalGames().then(ui.statsLoadSuccess).catch(ui.statsLoadFailure);
};
...
// incomplete games.
var onIncompleteShow = function onIncompleteShow(event) {
  event.preventDefault();
  var overTest = function overTest(input) {
    return input.over === false;
  };
  var incompleteGames = store.gamesStats.filter(overTest);
  for (var i = 0; i < incompleteGames.length; i++) {
    var vsPlayer = '';
    ...
    var gameId = incompleteGames[i].id;
...
var statsRedirect = function statsRedirect() {
  $('#two-player').hide();
...
var changePassword = function changePassword(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.changePasswordUser(data).then(ui.changePassSuccess).catch(ui.changePassFailure);
...
var beginGame = function beginGame() {
  gameApi.createGame().then(ui.beginGameSuccess).catch(ui.beginGameFailure);
};

// Creates a game vs an ai, stores the value, and redirects user to the ai game board.
var beginAiGame = function beginAiGame() {
  gameApi.createGame().then(ui.beginAiGameSuccess).catch(ui.beginAiGameFailure);
};

// Form for users on the same device is shown.
var sameDeviceForm = function sameDeviceForm() {
  $('#second-player-same').show();
  $('#second-player-form').show();
};

// User is directed to the two player section.
var twoPlayerRedirect = function twoPlayerRedirect() {
  $('#two-player').show();
  $('#instructions').hide();
  $('#game-board').hide();
  $('#change-pass').hide();
...
var secondPlayerSignIn = function secondPlayerSignIn(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  signInApi.signInUser(data).then(ui.secondPlayerSuccess).catch(ui.secondPlayerFailure);
  $('#second-player-form').find('input[type=text], textarea').val('');
  $('#second-player-form').find('input[type=password], textarea').val('');
};

var secondPlayerLocalBegin = function secondPlayerLocalBegin() {
  gameApi.createGame().then(ui.secondPlayerLocalSuccess).catch(ui.secondPlayerLocalFailure);
};
...
// Signs out user.
var signOutUser = function signOutUser(data) {
  return $.ajax({
...
var changePasswordUser = function changePasswordUser(data) {
  return $.ajax({
...
var beginGameSuccess = function beginGameSuccess(data) {
  store.game = data.game;
  store.turnCounter = 1;
  $('#profile').hide();
  $('#game-board').show();
...
var beginAiGameSuccess = function beginAiGameSuccess(data) {
  store.game = data.game;
  store.aiTime = false;
  store.turnCounter = 1;
  $('#profile').hide();
...
var beginGameFailure = function beginGameFailure() {
  var errorHtml = '<p>\n    <b>Oops!</b> There\'s been an error!\n    </p>\n    <p>Contact the\n    <a href="mailto:windmillwarrior@gmail.com">administrator</a>.</p>\n    ';
  $('#profile-error').html(errorHtml).addClass('scooch');
};

// Fails to begin a game.
var beginAiGameFailure = function beginAiGameFailure() {
  var errorHtml = '<p><b>Oops!</b> There\'s been an error!\n    </p>\n    <p>Contact the\n    <a href="mailto:windmillwarrior@gmail.com">administrator</a>.</p>\n    ';
  $('#profile-error').html(errorHtml).addClass('scooch');
};
...
// up to date.
var statsLoadSuccess = function statsLoadSuccess(data) {
  store.gamesStats = data.games;
  var overTest = function overTest(input) {
    return input.over === true;
  };
  var totalPlayed = data.games.length;
  var statHtml = '' + totalPlayed;
  $('#total-games').html(statHtml);
  var totalOver = data.games.filter(overTest);
  var totalIncomplete = totalPlayed - totalOver.length;
  statHtml = '' + totalIncomplete;
  $('#total-games-incomplete').html(statHtml);
  var winCalculation = function winCalculation(accumulator, currentValue) {
    if (win.xWinEvent(currentValue.cells) === true) {
      accumulator = accumulator + 1;
    }
    return accumulator;
  };
  var tieCalculation = function tieCalculation(accumulator, currentValue) {
    if (win.winEvent(currentValue.cells) === false) {
      accumulator = accumulator + 1;
    }
    return accumulator;
  };
  var totalWon = data.games.reduce(winCalculation, 0);
  statHtml = '' + totalWon;
  $('#total-games-won').html(statHtml);
  var totalTied = totalOver.reduce(tieCalculation, 0);
  statHtml = '' + totalTied;
  $('#total-games-tied').html(statHtml);
};

var statsLoadFailure = function statsLoadFailure(data) {
  var errorHtml = '<p>Hmm. Something is terribly wrong.</p>';
  $('#game-error').html(errorHtml);
};

var secondPlayerSuccess = function secondPlayerSuccess(data) {
  store.secondUser = data.user;
  var userGreet = store.secondUser.email.split('@');
...
var secondPlayerFailure = function secondPlayerFailure() {
...
module.exports = {
  ...
  beginGameFailure: beginGameFailure,
  beginGameSuccess: beginGameSuccess,
  statsLoadSuccess: statsLoadSuccess,
  statsLoadFailure: statsLoadFailure,
  beginAiGameFailure: beginAiGameFailure,
  beginAiGameSuccess: beginAiGameSuccess,
  secondPlayerSuccess: secondPlayerSuccess,
  secondPlayerFailure: secondPlayerFailure
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var guestEvents = __webpack_require__(7);

// Makes the board array reflect the move made on the HTML board.
// Initiated by the user.
// Uses the usual game update API, then goes to specialized UI success/failure.
var aiSetMove = function aiSetMove(move, index) {
  var arrayUpdate = {
    game: {
      cell: {
        index: index,
  ...
  var cellUpdate = JSON.stringify(arrayUpdate);
  api.updateGame(cellUpdate).then(ui.aiUpdateGameSuccess).catch(ui.aiUpdateGameFailure);
};

// Places the move on the HTML board and the array.
var aiBoardMove = function aiBoardMove(boxId) {
  var xHtml = '<p>X</p>';
  var boardCell = boxId.substring(1, 2);
  store.currentIndex = boardCell;
  // First, check to see if the array has been filled at the same index as the
  // data-cell-index of the div that was clicked. If not, proceed. Otherwise,
  // alert the user.
...
// This is the event that runs whenever a user clicks a board cell.
// Logs the space clicked, then checkes that the stored game is not over.
var aiClickEvent = function aiClickEvent() {
  // Create a string that can be used for jquery events.
  var boxId = '#' + this.id;
  // Check that the game is not over.
  if (store.game.over !== true) {
...
// Add click events to board.
var addAiBoardHandlers = function addAiBoardHandlers() {
...
module.exports = {
  addAiBoardHandlers: addAiBoardHandlers
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var aiMove = function aiMove(boardArray) {
  var aiChoice = null;
  var biggerThanFive = function biggerThanFive(number) {
    if (number > 5) {
      return true;
    } else {
      return false;
  ...
  var biggerThanThree = function biggerThanThree(number) {
    if (number > 3) {
      return true;
    } else {
      return false;
  ...
  var checkEven = function checkEven(number) {
    if (number % 2 === 0) {
      return true;
    } else {
      return false;
  ...
  var mapMaker = function mapMaker(aiArray) {
  ...
  for (var j = 0; j < players.length; j++) {
    if (boardArray[0] === players[j] && boardArray[1] === players[j] && boardArray[2] === '') {
      aiChoice = 2;
      return aiChoice;
    } else if (boardArray[1] === players[j] && boardArray[2] === players[j] && boardArray[0] === '') {
      aiChoice = 0;
      return aiChoice;
    } else if (boardArray[0] === players[j] && boardArray[2] === players[j] && boardArray[1] === '') {
      aiChoice = 1;
      return aiChoice;
    } else if (boardArray[0] === players[j] && boardArray[2] === players[j] && boardArray[1] === '') {
      aiChoice = 1;
    ...
    } else if (boardArray[3] === players[j] && boardArray[4] === players[j] && boardArray[5] === '') {
      aiChoice = 5;
      return aiChoice;
    } else if (boardArray[4] === players[j] && boardArray[5] === players[j] && boardArray[3] === '') {
      aiChoice = 3;
      return aiChoice;
    } else if (boardArray[3] === players[j] && boardArray[5] === players[j] && boardArray[4] === '') {
      aiChoice = 4;
      return aiChoice;
      // Check for bottom row.
    } else if (boardArray[6] === players[j] && boardArray[7] === players[j] && boardArray[8] === '') {
      aiChoice = 8;
      return aiChoice;
    } else if (boardArray[7] === players[j] && boardArray[8] === players[j] && boardArray[6] === '') {
      aiChoice = 6;
      return aiChoice;
    } else if (boardArray[6] === players[j] && boardArray[8] === players[j] && boardArray[7] === '') {
      aiChoice = 7;
      return aiChoice;
      // Check for first column.
    } else if (boardArray[0] === players[j] && boardArray[3] === players[j] && boardArray[6] === '') {
      aiChoice = 6;
      return aiChoice;
    } else if (boardArray[3] === players[j] && boardArray[6] === players[j] && boardArray[0] === '') {
      aiChoice = 0;
    } else if (boardArray[0] === players[j] && boardArray[6] === players[j] && boardArray[3] === '') {
      aiChoice = 3;
      return aiChoice;
      // Check for second column.
    } else if (boardArray[1] === players[j] && boardArray[4] === players[j] && boardArray[7] === '') {
      aiChoice = 7;
      return aiChoice;
    } else if (boardArray[4] === players[j] && boardArray[7] === players[j] && boardArray[1] === '') {
      aiChoice = 1;
      return aiChoice;
    } else if (boardArray[1] === players[j] && boardArray[7] === players[j] && boardArray[4] === '') {
      aiChoice = 4;
      return aiChoice;
      // Check for third row.
    } else if (boardArray[2] === players[j] && boardArray[5] === players[j] && boardArray[8] === '') {
      aiChoice = 8;
      return aiChoice;
    } else if (boardArray[5] === players[j] && boardArray[8] === players[j] && boardArray[2] === '') {
      aiChoice = 2;
      return aiChoice;
    } else if (boardArray[2] === players[j] && boardArray[8] === players[j] && boardArray[5] === '') {
      aiChoice = 5;
      return aiChoice;
      // Check for left diagonal.
    } else if (boardArray[0] === players[j] && boardArray[4] === players[j] && boardArray[8] === '') {
      aiChoice = 8;
      return aiChoice;
    } else if (boardArray[4] === players[j] && boardArray[8] === players[j] && boardArray[0] === '') {
      aiChoice = 0;
      return aiChoice;
    } else if (boardArray[0] === players[j] && boardArray[8] === players[j] && boardArray[4] === '') {
      aiChoice = 4;
      return aiChoice;
      // Check for right diagonal.
    } else if (boardArray[2] === players[j] && boardArray[4] === players[j] && boardArray[6] === '') {
      aiChoice = 6;
      return aiChoice;
    } else if (boardArray[4] === players[j] && boardArray[6] === players[j] && boardArray[2] === '') {
      aiChoice = 2;
      return aiChoice;
    } else if (boardArray[2] === players[j] && boardArray[6] === players[j] && boardArray[4] === '') {
      aiChoice = 4;
      return aiChoice;
    }
  }
  ...
  if (aiChoice === null) {
    var aiArray = boardArray.map(function (x) {
      return x;
    });
    mapMaker(aiArray);
    ...
    if (aiArray.some(biggerThanFive)) {
      ...
      // CHECK FOR NUMBERS!!!!!
      var max = aiArray.reduce(function (tot, cur) {
...
var aiUpdate = function aiUpdate(boardArray) {
  var html = '<p>O</p>';
  var aiChoice = aiMove(boardArray);
  ...
  api.updateGame(cellUpdate).then(aiUpdateGameSuccess).catch(aiUpdateGameFailure);
};

// Clear any errors from the page's display.
var aiMsgClear = function aiMsgClear() {
  $('#ai-user-msg').html('');
};

// Makes the API reflect that the game is over, shows reset button.
var aiMarkGameOver = function aiMarkGameOver(move, index) {
  var arrayUpdate = {
    game: {
      cell: {
        index: index,
  ...
  var cellUpdate = JSON.stringify(arrayUpdate);
  api.updateGame(cellUpdate).then(aiEndGameSuccess).catch(aiEndGameFailure);
};

var aiUpdateGameSuccess = function aiUpdateGameSuccess(data) {
  // Make the stored game reflect the updated API game.
  store.game = data.game;
  // Prepare variables if game is over.
  var move = store.currentPlayer.toLowerCase();
...
// Shows user a message if the game failed to update.
var aiUpdateGameFailure = function aiUpdateGameFailure(data) {
  var invalidMoveHtml = '<B>How bizzare.</B>';
  $('#ai-user-msg').html(invalidMoveHtml);
};

var aiEndGameSuccess = function aiEndGameSuccess(data) {
  // Make the stored game reflect the updated API game.
  store.game = data.game;
  // Tell player the game is over.
  var endMoveHtml = '<p><B>Game over.</B></p>';
...
var aiEndGameFailure = function aiEndGameFailure(data) {
  // Tell player something went wrong.
  var invalidMoveHtml = '<B>How bizzare.</B>';
  $('#user-msg').html(invalidMoveHtml);
};
...
module.exports = {
  aiUpdateGameFailure: aiUpdateGameFailure,
  aiUpdateGameSuccess: aiUpdateGameSuccess
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
